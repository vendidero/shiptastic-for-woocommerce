...
class ReturnShipment extends Shipment {
	...
	public function create_refund( $refund_args = array() ) {
		if ( $this->get_refund_order() ) {
			return new \WP_Error( 'refund_order_exists', _x( 'This return is already linked to a refund.', 'shipments', 'shiptastic-for-woocommerce' ) );
		}
		...
		if ( $order = $this->get_order() ) {
			...
			if ( $refund_total > 0 ) {
				$refund_reason = sprintf( _x( 'Refund to your return #%1$s.', 'shipments', 'shiptastic-for-woocommerce' ), $this->get_shipment_number() );
		...
		return new \WP_Error( 'refund_order_error', _x( 'There was an error creating a refund for this return.', 'shipments', 'shiptastic-for-woocommerce' ) );
	}
